...
public class RankingQueryService {
    ...
    public List<MonthlyRankDto> getMonthlyTop100WithTies(YearMonth ym) {
        var start = ym.atDay(1).atStartOfDay(ZoneId.of("Asia/Seoul")).toLocalDateTime();
        var end   = ym.plusMonths(1).atDay(1).atStartOfDay(ZoneId.of("Asia/Seoul")).toLocalDateTime();
    ...
    public MyRankDto getMyMonthlyRank(Long userId, YearMonth ym) {
        ...
        return list.stream()
                .filter(r -> java.util.Objects.equals(r.getUserId(), userId)) // NPE 방지
                .findFirst()
                .map(r -> new MyRankDto(
                        r.getUserId(),
                        r.getNickname(),
